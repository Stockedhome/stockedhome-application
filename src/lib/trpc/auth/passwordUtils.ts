...
const pepperBuffer = env.PASSWORD_PEPPER && Buffer.from(base64.toArrayBuffer(env.PASSWORD_PEPPER));
...
export async function hashPassword(password: string, salt?: Buffer): Promise<{ passwordSalt: ArrayBuffer; passwordHash: ArrayBuffer; }> {
    if (!pepperBuffer) throw new TypeError('No pepper set in the environment! This should not be possible unless you are hashing a password in codegen!');
